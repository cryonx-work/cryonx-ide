...
class CollabService {
    ...
    async connect(roomName: string, user: { name: string; color: string; isHost: boolean; id?: string }) {
        ...
        console.log(`[CollabService] Connecting to room: ${roomName}`);
    ...
    async checkSession(roomName: string): Promise<boolean> {
        ...
        console.log(`[CollabService] Checking session: ${roomName}`);
            ...
            try {
                ...
                const cleanup = () => {
                    ...
                    try {
                        ...
                        console.warn("Error destroying temp provider", e);
                    }
                };

                const timeout = setTimeout(() => {
                    console.log("[CollabService] Check session timeout");
                    cleanup();
                    resolve(false);
                ...
                tempProvider.awareness.on('change', () => {
                    ...
                    if (foundHost) {
                        console.log("[CollabService] Host found");
                ...
                console.error("[CollabService] Error in checkSession", error);
                resolve(false);
            }
        });
    }
    ...
    pushProject() {
        ...
        if (!files || files.length === 0) {
            console.warn("[CollabService] No project data to push.");
            return;
        }
        ...
        if (yFiles.length > 0) {
            console.log("[CollabService] Doc already initialized. Skipping seed.");
            return;
        }

        console.log("[CollabService] Seeding project -> Y.Doc");
        ...
        console.log("[CollabService] Project seeded.");
    }
    ...
    bindFileSystem() {
        ...
        const updateFileSystem = () => {
            console.log("[CollabService] File tree updated from remote");
            const newItems: any[] = [];
        ...
        yFiles.observeDeep((events, transaction) => {
            if (transaction.local) {
                console.log("[CollabService] Ignoring local file tree update");
                return;
            }
            updateFileSystem();
        });
        ...
        if (yFiles.length > 0) {
            console.log("[CollabService] Initial file system sync");
            updateFileSystem();
        }
    }
    ...
    private async _cleanupOldGuestDatabases(currentRoomName: string, forceClearCurrent: boolean = false) {
        ...
        try {
            ...
            if (dbsToDelete.length > 0) {
                console.log('[CollabService] Cleaning up guest sessions...', dbsToDelete.map(d => d.name));
                await Promise.all(dbsToDelete.map(db => {
                    return new Promise<void>((resolve) => {
                        if (!db.name) return resolve();
                        const req = window.indexedDB.deleteDatabase(db.name);
                        req.onsuccess = () => {
                            console.log(`[CollabService] Deleted DB: ${db.name}`);
                            resolve();
                        };
                        req.onerror = () => {
                            console.warn(`[CollabService] Failed to delete DB: ${db.name}`);
                            resolve();
                        };
                        req.onblocked = () => {
                            console.warn(`[CollabService] Delete blocked for DB: ${db.name}`);
                            resolve();
                        };
                    });
                }));
            ...
            console.warn('[CollabService] Error cleaning up databases:', error);
        }
    }
}
