...
export const useProjectStore = create<ProjectState>((set, get) => ({
    ...
    init: async () => {
        try {
            ...
            if (activeProjectId) {
                console.log("[ProjectStore] Restoring active project:", activeProjectId);
                // Don't await this to speed up initial load
                get().loadProjectData(activeProjectId);
            } else {
                console.log("[ProjectStore] No active project found in config");
    ...
    loadProjectData: async (projectId) => {
        ...
        try {
            ...
            const projectFiles = await projectService.ensureProjectInitialized(projectId);
            ...
            if (savedMeta) {
                ...
                if (savedMeta.git && savedMeta.git.isInitialized) {
                    // Don't optimistically set initialized. Let refreshStatus verify the .git folder.
                    // git.setIsInitialized(true);

                    // Refresh git status from FS
                    git.refreshStatus().catch(e => console.warn("[ProjectStore] Failed to refresh git status", e));
                } else {
            ...
            let currentProject = get().allProjects.find(p => p.id === projectId);
            if (!currentProject) {
                console.warn(`[ProjectStore] Project ${projectId} not found in project list. Creating temporary entry.`);
                currentProject = {
                    id: projectId,
                ...
                // Add to list to persist recovery
                set(state => ({ allProjects: [...state.allProjects, currentProject!] }));
                // Trigger save of project list
                projectService.saveProjects([...get().allProjects, currentProject]).catch(console.error);
            }
    ...
    renameProject: (projectId, newName) => {
        set((state) => {
            ...
            projectService.saveProjects(updatedProjects).catch(e =>
                console.error("Failed to save renamed project list", e)
            );
            return { allProjects: updatedProjects };
        });
    },
    ...
    deleteProject: (projectId) => {
        ...
        if (window.confirm(`Delete project "${projectToDelete.name}" permanently?`)) {
            projectService.deleteProject(projectId).catch(e =>
                console.error("[ProjectStore] Failed to delete project from Service", e)
            );
            ...
            projectService.saveProjects(newProjects).catch(e =>
                console.error("Failed to save project list after delete", e)
            );
